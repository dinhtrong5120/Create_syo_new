...
from src.ux_ui.user_read_only_prokan import view_prokan
from src.app.update_file import get_csrf_token, update_file_into_server
        ...
        with st.form('input_form'):
            ...
            if files is not None and col_left_spec_grid.form_submit_button("Load File",
                ...
                with st.spinner(text="In progress..."):
                    ...
                    set_data("running", 0)
            # ==============================================================================================================================
            if col_left_spec_grid.form_submit_button("View Group", use_container_width=True) and get_data(
                    "running") == 0:
                with st.spinner(text="In progress..."):
                    list_group, notice = get_name_group(st.session_state.code)
                    list_group = ["ALL"] + list_group
            ...
            if col_left_spec_grid.form_submit_button("Create Cadics old",
                ...
                with st.spinner(text="In progress..."):
                    notice, session, data, project_id, app_list = create_cadics_old(st.session_state.case,
            ...
            if col_left_spec_grid.form_submit_button("Create Cadics new",
                                                     use_container_width=True) == True and get_data("running") == 0:
                # if st.form_submit_button("Confirm Create Cadics", use_container_width=True)==True:
                set_data("running", 1)
                # st.write(st.session_state.case,st.session_state.plant,st.session_state.pwt,st.session_state.code)
            ...
            if col_left_spec_grid.form_submit_button("Update File Cadics", use_container_width=True) and get_data(
                    "running") == 0:
                set_data("running", 1)
                with st.spinner(text="In progress..."):
