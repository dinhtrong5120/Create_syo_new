...
def admin_all_1():
    ...
    modal_create_syo = Modal(key="modal_create_syo", title="Create syo...")
            ...
            with modal_load_file.container():
                with st.spinner(text="In progress..."):
                    if len(files) > 0:
                        ...
                        # st.write(files)
                        # st.write("st.session_state.code: ", st.session_state.code == "")
                        update_syo()
                    ...
                    if files is not None and files != []:
                        ...
                        if code not in str(files[0].name).lower() or 'Spec_List' not in files[0].name:
                            ...
                            if flg_check_fail:
                                ...
                                # st.write('data_for_create: ', data_for_create[0])
    ...
    with col_right:
        ...
        with col_r1:
            st.markdown(
                '<h1 style="text-align: center;background-color: #CCE9D9;border: 2px solid #4CAF50; padding: 10px;">プロ管集約業務システム</h1>',
                unsafe_allow_html=True)
            #    st.markdown(
        # '<h1 style="text-align: center;background-color: #CCE9D9;border: 2px solid #4CAF50; padding: 10px;">INPUT</h1>',
        # unsafe_allow_html=True)
